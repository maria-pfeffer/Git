...
# Länder, die in einem Datensatz fehlen, sollten einen NA-Wert in dieser Spalte haben.laenderdaten) <- merge(ausgaben, mortalitaet, by = "country", all = TRUE)
colnames(ausgaben) <- c("country", "spending")
colnames(mortalitaet) <- c("COUNTRY", "mort")
